...
func TestExampleGoViperCliHelloWorldSuccess(t *testing.T) {
  ...
  output_path           = "output"`, 1)
	...
	modifiedContent = strings.Replace(modifiedContent, `  source = "../stacks/tofukit-stack-go-viper-cobra-pterm"`, `  source = "./stacks/tofukit-stack-go-viper-cobra-pterm"`, 1)
	...
	t.Run("InitialCreation", func(t *testing.T) {
		...
		assert.Contains(t, string(rootContent), "github.com/pterm/pterm", "root.go should import pterm")
		...
		require.NoError(t, err)
		assert.Contains(t, string(goModContent), "example.com/go-hello", "go.mod should contain module path")
		...
		assert.Contains(t, string(readmeContent), "go-hello",
			"README should contain project name 'go-hello' (from stack files)")
		t.Log("✓ README.md contains 'go-hello' as expected from stack merge")
	...
	t.Run("DebugFilesVerification", func(t *testing.T) {
		...
		if len(debugSpecFiles) > 0 {
			...
			require.NoError(t, err)
			assert.Contains(t, string(spec), "go-hello")
		...
		if len(mdFiles) > 0 {
			...
			if err == nil {
				assert.Contains(t, string(mdContent), "go-hello")
